...
import { SettingVo } from "app/setting/schema";
import { SettingCreateModal } from "components/setting/setting-create-modal";
...
export default function Setting() {
  ...
  const [data, setData] = useState<SettingVo[]>([]);
  ...
  const fetchData = async (accessToken: string) => {
    try {
      ...
      const data = json.data as SettingVo[];
      if (data) {
        setData(data);
        setIsLoading(false);
      }
    ...
    <section className={"setting"}>
      ...
      <div className={"table"}>
        <Table>
          ...
          <TableBody>
            {data.map((row: SettingVo, index: number) => (
              <>
                <tr key={index}>
                  <td>{index + 1}</td>
                  <td>
                    {row.startDate === "10000000"
                      ? "-"
                      : formatDate(row.startDate)}
                  </td>
                  <td>~</td>
                  <td>
                    {row.endDate === "99999999" ? "-" : formatDate(row.endDate)}
                  </td>
                  <td>{row.ratio}</td>
                  <td>
                    <Button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          const response = await fetch(
                            `${process.env.NEXT_PUBLIC_BACK_URL}/setting?id=${row.settingId}`,
                            {
                              method: "DELETE",
                              headers: {
                                "Content-Type": "application/json",
      ...
      <SettingCreateModal open={createOpen} setOpen={setCreateOpen} />
    </section>
  );
}
